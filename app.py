...
.main-header h1 { font-size: 3.2rem; font-weight: 800; background: linear-gradient(135deg, #00d4ff 0%, #0099cc 50%, #00ff88 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: -1px; margin: 0; }
.main-header p { color: #64748b; font-size: 1rem; margin-top: 0.5rem; letter-spacing: 2px; text-transform: uppercase; }
...
def overlay_gradcam(img_array_orig, heatmap, target_size):
    ...
    return cv2.addWeighted(img.astype(np.uint8), 0.6, heatmap_colored, 0.4, 0)
...
with st.sidebar:
    ...
    for label in MODEL_CONFIG[modality_key]['class_labels']:
        st.markdown(f"<div style='font-size:0.85rem;color:#94a3b8;padding:2px 0'>→ {label}</div>", unsafe_allow_html=True)
    st.markdown("<hr class='divider'>", unsafe_allow_html=True)
    st.markdown("<div style='font-size:0.75rem;color:#334155'>⚠️ For research purposes only.<br>Not a substitute for professional medical diagnosis.</div>", unsafe_allow_html=True)
...
st.markdown(f"<div style='text-align:center;margin-bottom:2rem'><span class='badge {badge_class}'>{modality}</span></div>", unsafe_allow_html=True)
...
if uploaded_file:
    ...
    with col2:
        ...
        if result['gradcam_img'] is not None:
            ...
            st.markdown("<div style='font-size:0.75rem;color:#475569;text-align:center'>🔴 Red = High attention &nbsp;|&nbsp; 🔵 Blue = Low attention</div>", unsafe_allow_html=True)
        else:
    ...
    with col3:
        ...
        if confidence < 70:
            st.markdown("<div class='warning-box'>⚠️ <strong>Low confidence.</strong><br>Please consult a radiologist.</div>", unsafe_allow_html=True)
